...
// boot中已经初始化完成了，这里就无需再次初始化。
    cyg_uint32 baud_value = 0;
    cyg_uint32 baud_rate  = ((channel_data_t*)__ch_data)->baud_rate;

	// 把DTX DRX设置有效
	HAL_WRITE_UINT32(AT91C_PIOB_ASR, ((AT91C_PB14_DRXD | AT91C_PB15_DTXD)));
	HAL_WRITE_UINT32(AT91C_PIOB_PDR, ((AT91C_PB14_DRXD | AT91C_PB15_DTXD)));

    /* Disable interrupt */
    HAL_WRITE_UINT32((base+AT91_DBG_IDR), 0xFFFFFFFF);

    /* Reset receiver and transmitter */
    HAL_WRITE_UINT32((base+DBGU_CR) , (AT91C_US_RSTRX | AT91C_US_RSTTX | AT91C_US_RXDIS | AT91C_US_TXDIS));

	baud_value = (cyg_uint32)(((hal_pcm_mck / ( baud_rate << 3 )) + 1)/2);
	...
	HAL_WRITE_UINT32((base+DBGU_MR),(AT91C_US_PAR_NONE | AT91C_US_CHMODE_NORMAL));

    /* Enable Transmitter */
    HAL_WRITE_UINT32((base+DBGU_CR), AT91C_US_TXEN);

    /* Enable Receiver */
    HAL_WRITE_UINT32((base+DBGU_CR), AT91C_US_RXEN);
#endif
}
...
//       	random_device_identification_generator();
        return true;
    }
    return false;
}
...
static channel_data_t at91_ser_channels[1] = {
    { (cyg_uint8*)AT91_DBG, 10000, CYGNUM_HAL_INTERRUPT_DBG,
